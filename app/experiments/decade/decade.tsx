...
export function Decade({ initialActive = 3 }: { initialActive?: number }) {
  ...
  return (
    ...
    >
      ...
      {/* up / down controls — for touch, and as an arrow-key alternative */}
      <div className="fixed bottom-6 left-6 z-40 flex flex-col gap-2 sm:left-8">
        <button
          type="button"
          onClick={() => go(active - 1)}
          disabled={active === 0}
          aria-label="Earlier year"
          className="grid size-10 place-items-center rounded-full border border-border bg-card text-text-secondary shadow-sm transition-[color,border-color,transform] duration-[var(--dur-press)] ease-out hover:border-border-strong hover:text-foreground active:scale-90 disabled:pointer-events-none disabled:opacity-30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
        >
