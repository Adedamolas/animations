...
export function createBookBuilder(disposables) {
  ...
  function buildBook(cfg, i, parent) {
    ...
    return {
      ...
      slotScale: 1,
      exit: null,
      exitVel: 0, // speed of the authored exit curve, for the smear
      orbY: 0,
      orbYv: 0,
      orbPhase: "idle",
      orbTarget: 0,
...
/* Stretch per scene-unit/second of exit speed, and the ceiling. The glide out
   peaks around 8.8 units/s, so it tops out near the cap for the fastest part
   of the move and eases off as it settles — which is the whole point. */
const SMEAR = 0.017;
const SMEAR_MAX = 0.16;
...
export function tickBook(b, ctx, dt, t) {
  ...
  /* Smear, not blur. A book leaving frame is perfectly sharp at speed, and
     sharpness at speed reads as weightless — but the books are WebGL, so a
     real blur would cost a whole post-processing pass for something you would
     barely register. Stretching along the direction of travel and pinching
     across it is what the eye actually reads as motion, and it is free.
     Volume is roughly preserved, so it looks like speed rather than growth. */
  const smear = clamp(Math.abs(b.exitVel || 0) * SMEAR, 0, SMEAR_MAX);
  b.root.scale.set(s.sc.v * (1 - smear * 0.5), s.sc.v * (1 + smear), s.sc.v * (1 - smear * 0.5));
